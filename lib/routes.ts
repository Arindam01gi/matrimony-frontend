export const DISCOVERY_PATH = "/discovery";
export const LIKES_PATH = "/likes";
export const CHAT_PATH = "/chat";
export const PROFILE_DETAILS_PATH = "/profile-details";
export const PARTNER_PREFERENCE_PATH = "/partner-preference";
export const MY_PROFILE_PATH = "/my-profile";
export const EDIT_PROFILE_PATH = "/edit-profile";
export const NOTIFICATIONS_PATH = "/notifications";
export const ONBOARDING_PATH = "/onboarding?step=personal";

export const AUTH_LOGIN_PATH = "/auth?mode=login";
export const AUTH_REGISTER_PATH = "/auth?mode=register";

export const AUTH_API_LOGIN_PATH = "/api/auth/login";
export const AUTH_API_REGISTER_PATH = "/api/auth/register";
